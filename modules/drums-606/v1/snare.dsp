...
decay=hslider("decay[unit:s][scale:log][col:0][row:0]",0.2,0.025,1.2,.001);
tone=hslider("tone[col:1][row:0]",.5,0,1,.001):u.sm;
snappy=hslider("snappy[col:2][row:0]",0.65,0,1,.001):u.sm;
level=hslider("level[col:3][row:0]",.8,0,1,.001):u.sm;
h=gate>gate';
f=u.lat(h,freq);d=u.lat(h,decay);v=u.lat(h,velocity);a=u.lat(h,accent);
body=h:u.ring(f,d*.46);
wire=no.noise:fi.highpass(2,700*pow(4,tone)):fi.lowpass(1,min(ma.SR*.4,12500));
raw=.6*(1-.65*snappy)*body+( .09+.60*snappy)*wire*u.env(h,d,.00028);
