declare name "TX81Z OPZ panel-frequency candidate";
declare version "0.6.0-panel-fixed-frequency";
declare description "TX81Z-facing fixed-frequency semantics over the qualified OPZ v5 native-law core";
import("stdfaust.lib");
opz=library("../v5/opz_core.lib");
panel=library("panel_frequency.lib");
...
algorithm=nentry("algorithm",1,1,8,1);
feedback=nentry("feedback",0,0,7,1);
level=hslider("level",0.5,0,1,.001);
blockFreq=nentry("blockFreq",-1,-1,8191,1);
bf=int(select2(blockFreq>=0,opz.fromHz(freq),blockFreq));
tick=opz.egClock:(_,!); count=opz.egClock:(!,_);
a=int(algorithm)-1;
...
p1DT1=nentry("op1DT1",0,0,7,1); p1DT2=nentry("op1DT2",0,0,3,1); p1Range=nentry("op1Range",0,0,7,1); p1TL=nentry("op1TL",0,0,127,1);
...
eg1=opz.envelope(gate,tick,count,bf,p1AR,p1D1R,p1D2R,p1SL,p1RR,p1KS,p1Reverb); att1=eg1:(_,!); state1=eg1:(!,_);
...
p2DT1=nentry("op2DT1",0,0,7,1); p2DT2=nentry("op2DT2",0,0,3,1); p2Range=nentry("op2Range",0,0,7,1); p2TL=nentry("op2TL",36,0,127,1);
...
eg2=opz.envelope(gate,tick,count,bf,p2AR,p2D1R,p2D2R,p2SL,p2RR,p2KS,p2Reverb); att2=eg2:(_,!); state2=eg2:(!,_);
...
p3DT1=nentry("op3DT1",0,0,7,1); p3DT2=nentry("op3DT2",0,0,3,1); p3Range=nentry("op3Range",0,0,7,1); p3TL=nentry("op3TL",36,0,127,1);
...
eg3=opz.envelope(gate,tick,count,bf,p3AR,p3D1R,p3D2R,p3SL,p3RR,p3KS,p3Reverb); att3=eg3:(_,!); state3=eg3:(!,_);
...
p4DT1=nentry("op4DT1",0,0,7,1); p4DT2=nentry("op4DT2",0,0,3,1); p4Range=nentry("op4Range",0,0,7,1); p4TL=nentry("op4TL",36,0,127,1);
...
eg4=opz.envelope(gate,tick,count,bf,p4AR,p4D1R,p4D2R,p4SL,p4RR,p4KS,p4Reverb); att4=eg4:(_,!); state4=eg4:(!,_);
