...
// TX81Z VCED LFO controls. Wave: 0 saw, 1 square, 2 triangle, 3 sample/hold.
...
pm=lfo.pmDelta(lfoSpeed,lfoDelay,pmd,pms,lfoWave,lfoSync,gate);
modFreq=freq*pow(2.0,float(pm)/768.0);
bf=int(select2(blockFreq>=0,opz.fromHz(modFreq),blockFreq));
am=lfo.amOffset(lfoSpeed,lfoDelay,amd,ams,lfoWave,lfoSync,gate);
...
eg1=opz.envelope(gate,tick,count,bf,p1AR,p1D1R,p1D2R,p1SL,p1RR,p1KS,p1Reverb); att1=min(1023,(eg1:(_,!))+int(am)*int(p1AME)); state1=eg1:(!,_);
...
eg2=opz.envelope(gate,tick,count,bf,p2AR,p2D1R,p2D2R,p2SL,p2RR,p2KS,p2Reverb); att2=min(1023,(eg2:(_,!))+int(am)*int(p2AME)); state2=eg2:(!,_);
...
eg3=opz.envelope(gate,tick,count,bf,p3AR,p3D1R,p3D2R,p3SL,p3RR,p3KS,p3Reverb); att3=min(1023,(eg3:(_,!))+int(am)*int(p3AME)); state3=eg3:(!,_);
...
eg4=opz.envelope(gate,tick,count,bf,p4AR,p4D1R,p4D2R,p4SL,p4RR,p4KS,p4Reverb); att4=min(1023,(eg4:(_,!))+int(am)*int(p4AME)); state4=eg4:(!,_);
