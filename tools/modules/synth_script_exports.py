"""Make single-file script delivery with the real Faust expansion pass.

Review exports are immutable snapshots, not automatic factory promotion or an
assertion that unfinished hardware/listening acceptance has passed.
"""
from pathlib import Path
import argparse, json, os, re, shutil, tempfile, traceback
import numpy as np
...
def run(out):
    lab=SynthLab(out);(out/'scripts').mkdir(exist_ok=True);(out/'audition').mkdir(exist_ok=True)
    c=lab.check
    ...
    try:
        for name,(path,base) in SUBJECTS.items():
            ...
            with tempfile.TemporaryDirectory(prefix='faust-single-script-') as temp:
                work=Path(temp);wrapper=work/(name+'.dsp')
                wrapper.write_text('import('+json.dumps(str(src))+');\n')
                invocation=['faust','-e','-I',src.parent,'-I',COMMON,'-lang','cpp',wrapper,'-o',work/'unused.cpp']
                command(invocation)
                files=list(work.glob('*-exp.dsp'))
                if len(files)!=1:raise RuntimeError('Expected one expanded Faust source, found '+str(list(work.iterdir())))
                expanded=files[0].read_text()
                ...
                standalone=build/'render';c(name+':same-controls',controls(canonical)==controls(standalone))
                maxerr=0.0
                for sr in (44100,48000,96000):
                    events=[(round(.1*sr),'gate',1),(round(.5*sr),'cutoff',800),
                        (round(.8*sr),'freq',440),(round(1.1*sr),'gate',0),(round(1.2*sr),'cutoff',300)]
                    p=base|{'release':.6}
                ...
                manifest['subjects'][name]=dict(canonical_path=path,canonical_sha256=digest(src),
                    exported_file=name+'.dsp',exported_sha256=digest(destination),
                    audio_sha256=digest(out/'audition'/(name+'-review-phrase.wav')),
                    expanded_audio_max_error=maxerr,reference_and_listening_status='See the exact checkpoint PR; no implicit approval')
        c('all-four-script-exports',len(manifest['subjects'])==4)
    except Exception as e:
        lab.report['exception']=traceback.format_exc();c('export-completed',False)
    ...
    (out/'scripts'/'manifest.json').write_text(json.dumps(manifest,indent=2))
