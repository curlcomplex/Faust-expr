...
int main(int argc,char** argv){try{
 ...
 constexpr int count=160,warm=32;
 int voices=argc>3?int(integer(argv[3])):4;
 if(voices<1||voices>32)throw std::runtime_error("voice count");
 // Initialize the shared bank ONCE, before any voice can render.
 ...
 std::cout<<std::setprecision(12)<<"{\"frames\":"<<b<<",\"host_voices\":"<<voices<<",\"class_init_us\":"<<classInitUs<<",\"stack\":"<<stack<<",\"rate\":48000,\"object_bytes\":"<<sizeof(ModuleDSP)<<",\"ordinary_new_in_compute\":"<<allocations.load()<<",\"p50_us\":"<<d[count/2]<<",\"p95_us\":"<<d[int(count*.95)]<<",\"p99_us\":"<<d[int(count*.99)]<<",\"max_us\":"<<d.back()<<",\"checksum\":"<<checksum<<"}\n";
