...
def classify_compile(result):
 ...
 return {'hard_failure':result['returncode']!=0,'warning_present':'warning' in text.lower(),
...
def diagnose(dsp,faust,libs,out,tracer=None,trace=4,structural=False):
 ...
 if tracer:
  # -input makes the upstream dummy driver exercise effect inputs with its test signals;
  # without it an input-dependent fault can remain unexecuted until timeout.
  tr=run([tracer,'-trace',str(trace),'-input','-noui','-timeout','2','-I',libs,dsp],timeout=15)
  result['interp_tracer']={**tr,'trace_mode':trace,'input_test_signals':True,'hard_failure':tr['returncode']!=0,
   'policy':'trace 4 fatal FP infinite/NaN/cast/load-store faults are hard diagnostics; other counters remain evidence'}
 if structural:
  help_result=run([faust,'-h']);supported='-sig' in (help_result['stdout']+help_result['stderr'])
  ...
  if supported: sr['result']=run([faust,'-lang','ocpp','-sig','-I',libs,dsp,'-o',out/'structural.cpp'])
...
def main():
 ...
 print(json.dumps({'report':str(a.out/'diagnostics.json'),'compiler_hard_failure':r['compiler_classification']['hard_failure'],'tracer_hard_failure':None if r['interp_tracer'] is None else r['interp_tracer']['hard_failure']}))
