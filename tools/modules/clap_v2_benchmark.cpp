// Same-sound warmed four-voice benchmark. No device or whole-allocator claim.
#include <atomic>
#include <cstdlib>
#include <new>
...
int main(int argc,char**argv){try{
 const int block=argc>1?int(integer(argv[1])):128;if(block<1||block>2048)throw std::runtime_error("block size");
 ...
 const auto ns=std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now()-start).count();allocationGuard=false;if(j>=0)timings.push_back(float(ns)/1000.f);}
 if(!std::isfinite(checksum))throw std::runtime_error("nonfinite compute");std::sort(timings.begin(),timings.end());
 std::cout<<std::setprecision(12)<<"{\"voices\":4,\"rate\":48000,\"block\":"<<block<<",\"iterations\":"<<iterations<<",\"sizeof_dsp_bytes\":"<<sizeof(ModuleDSP)<<",\"ordinary_new_allocations\":"<<allocationCount.load()<<",\"p50_us\":"<<timings[iterations/2]<<",\"p95_us\":"<<timings[int(iterations*.95)]<<",\"p99_us\":"<<timings[int(iterations*.99)]<<",\"max_us\":"<<timings.back()<<",\"checksum\":"<<checksum<<"}\n";return allocationCount?1:0;
