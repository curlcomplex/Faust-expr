...
def main(out=OUT_DEFAULT):
    ...
    failures=[x for x in L.report.get('checks',[]) if not x.get('passed',False)]
    L.report['failures']=failures
    (out/'report.json').write_text(json.dumps(L.report,indent=2,sort_keys=True))
    print(json.dumps({'checks':len(L.report.get('checks',[])),'failures':[f['name'] for f in failures],'out':str(out)},indent=2))
    if failures:
        raise SystemExit(1)
