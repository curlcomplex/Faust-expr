...
def render(exe,out,label,params,events=None,seconds=2,block=128,rate=48000):
 defaults={'pitch_hz':220,'morph':.28,'shape':.45,'detune':.18,'stack':2,'decay':.55,'drive':.08,'velocity':1,'gate':0}
 values=defaults.copy();values.update(params); rows=[(0,k,v) for k,v in values.items()]+list(events or []);rows.sort()
...
def main():
 ...
 for i in range(32):
  p={'morph':float(i&1),'shape':float((i>>1)&1),'detune':float((i>>2)&1),'stack':1+((i>>3)&3),'decay':float((i>>1)&1),'drive':float((i>>4)&1),'pitch_hz':80 if i&1 else 1200}
