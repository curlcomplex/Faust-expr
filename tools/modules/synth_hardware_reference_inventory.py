...
Only provenance, archive hashes, filenames, small text documentation and derived
WAV descriptors are retained. Commercial/purchase-gated corpora remain external
...
NOTES = {
 "juno60_hornet_1997": "Legacy Juno-60 hardware WAV archive; use included documentation where present. Absolute recording gain/control calibration is not assumed.",
 "juno106_soundwave_1_1997": "Legacy Juno-106 hardware WAV archive; documentation/settings retained where present.",
 "juno106_soundwave_2_1997": "Second legacy Juno-106 hardware WAV archive; documentation/settings retained where present.",
...
def mono_float(sr,x):
    x=np.asarray(x); channels=1 if x.ndim==1 else int(x.shape[1])
    ...
    return int(sr),channels,x

def desc(sr,x):
    if len(x)==0: return {"frames":0}
    a=np.abs(x); peak=float(a.max()); rms=float(np.sqrt(np.mean(x*x))); y=x[:min(len(x),sr*4)]
    if np.max(np.abs(y))>0:
        f,p=welch(y,fs=sr,nperseg=min(8192,len(y))); ps=float(p.sum())
    ...
    e=x*x; total=float(e.sum()); e90=float(np.searchsorted(np.cumsum(e),.90*total)/sr) if total>0 else 0.0
...
def run(out):
    ...
    report={"policy":"No reference WAV/archive bytes retained. Small included text docs and descriptors only. Diagnostics are not authenticity scores.","sources":{}}
    success=0
    for name,urls in SOURCES.items():
        ...
        try:
            ...
            z=zipfile.ZipFile(io.BytesIO(data)); rows=[]; docs=[]
            for fn in z.namelist():
                raw=z.read(fn)
                if fn.lower().endswith((".txt",".nfo",".diz",".doc","readme")) and len(raw)<=200000:
                    txt=decode_doc(raw)
                    if txt is not None: docs.append({"file":fn,"sha256":sha(raw),"text":txt})
                if not fn.lower().endswith((".wav",".wave")): continue
                try:
                    sr,ch,x=mono_float(*wavfile.read(io.BytesIO(raw)))
                    rows.append({"file":fn,"wav_sha256":sha(raw),"sample_rate":sr,"channels":ch,**desc(sr,x)})
                except Exception as e: rows.append({"file":fn,"wav_sha256":sha(raw),"error":str(e)})
            item["wav_count"]=len(rows); item["wavs"]=rows; item["documents"]=docs; success+=bool(rows)
    ...
    print(json.dumps({k:{"wav_count":v.get("wav_count",0),"documents":[d["file"] for d in v.get("documents",[])],"error":v.get("error")} for k,v in report["sources"].items()},indent=2))
