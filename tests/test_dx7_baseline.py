...
import dx7_baseline as dx


class DX7BaselineTests(unittest.TestCase):
    def setUp(self):
        self.suite=json.loads((ROOT/'modules/dx7/baseline-01/cases.json').read_text())

    def test_schema_and_family_counts(self):
        dx.validate_suite(self.suite)
        self.assertEqual([sum(c['family']==f for c in self.suite['cases']) for f in ['C01','C02','C03']],[5,3,1])
    ...
    def test_faust_uses_actual_function_order_not_stale_usage_comment(self):
        s=dx.faust_source(self.suite['cases'][6],Path('/pinned'))
