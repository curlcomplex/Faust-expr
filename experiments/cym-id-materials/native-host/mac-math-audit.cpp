// Apple Silicon phase-math diagnostic. No cymbal/model data are substituted here.
// This tests execution candidates against this Mac's ordinary double libm.
// It is NOT a whole-instrument benchmark or a sound-equivalence claim.
...
// Volatile function pointers forbid compiler replacement/fusion in the reference.
...
static void compare(Check&ch,const std::vector<double>&x,Fn fn){
 const int n=int(x.size());std::vector<double>rs(n),rc(n),s(n),c(n);ordinary(n,x.data(),rs.data(),rc.data());fn(n,x.data(),s.data(),c.data());
...
int main(){
 std::cout<<std::setprecision(17);Check pc,vc;
 // Frozen engine uses nonnegative phase increments up to 0.96*pi.
 for(int batch=0;batch<32;batch++) {std::vector<double>x(65536);for(double&v:x)v=uniform()*(0.96*M_PI);compare(pc,x,paired);compare(vc,x,force);}
 ...
 compare(pc,bound,paired);compare(vc,bound,force);
 ...
 ordinary(int(x.size()),x.data(),new double[x.size()],new double[x.size()]);
 std::cout<<"{\"scope\":\"Mac platform double-precision phase-math diagnostic only; not full model or DAW timing\",\"range\":[0,"<<0.96*M_PI<<"],\"checks\":{";printcheck("paired_sincos",pc);std::cout<<',';printcheck("accelerate_vvsincos",vc);std::cout<<"},\"timings\":[";
 bool first=true;for(int block:{64,128,256})for(int repeat=0;repeat<4;repeat++){
  double a,b,c;if(repeat%2){c=timed(force,x,block,80);b=timed(paired,x,block,80);a=timed(ordinary,x,block,80);}else{a=timed(ordinary,x,block,80);b=timed(paired,x,block,80);c=timed(force,x,block,80);}
  if(!first)std::cout<<',';first=false;std::cout<<"{\"block\":"<<block<<",\"repeat\":"<<repeat<<",\"pairs\":"<<x.size()*80<<",\"scalar_seconds\":"<<a<<",\"paired_seconds\":"<<b<<",\"vforce_seconds\":"<<c<<'}';
 }
